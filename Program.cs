...
class Program
{
    static void Main(string[] args) //args giver mulighed for at modtage input fra kommandolinjen istedet for Console.ReadLine() feks.
    {
        /*
        BiloplysningsApp
        ...
        //Varablerne er ændret fra eksplicitte typer til implicitte typer med var 
        var bilMærke = ""; //Jeg har fjernet mellemrum i "" da det støjer
        var bilModel = ""; 
        // Hvad med motorstørrelse??
        var bilÅrgang = 0;
        var gearType = ' '; //Her skal der dog være mellemrum, man kunne også indsætte et X der viser man ikke har sat noget ind
        
        //Variablerne med udvidet biloplysninger Opg.2
        var brændstofType = "";
        var kilometerPerLitre = 0.0;
        var brændstofPris = 0.0;
        var kilometerStand = 0; //Burde denne være double??? Nogen skriver komma istedet for punktum! //Denne er koblet sammen med kørtDistance
        
        //Endnu en udvidelse Opg2.1.2
        var kørtDistance = 0; //kørertur
        var brændstofbehov = 0.0;
        var turPris = 0.0;
        var kilometerstandEfter = 0;
        
        //Dette er en introtekstˇ
            Console.WriteLine("=== Velkommen til biloplysningsprogrammet ==="); //Laver tydelig header med ===
        ...
        //Skriver Variablerne Ud
            Console.WriteLine($"Indtast dit bilmærke:");
                bilMærke = Console.ReadLine(); // Brug Console.ReadLine til at læse brugerens input og gemme det i de relevante variabler.
        ...
        //Konvertere det til int ved brug af Convert.ToInt32(Console.ReadLine()).
            Console.WriteLine($"Indtast bilens årgang:");
                bilÅrgang = Convert.ToInt32(Console.ReadLine()); //Hvis brugeren indtaster noget, der ikke er et tal, vil Convert.ToInt32() give en fejl. Bedre at bruge int.TryParse()
            ...
            Console.WriteLine($"Indtast bilens brændstoftype D for Diesel eller B for Benzin:"); //Hvad med at tilføje hybrid eller EL???
                brændstofType = Console.ReadLine();
                
            Console.WriteLine("Indtast hvad bilen kører per liter:");
                kilometerPerLitre = Convert.ToDouble(Console.ReadLine()); //Pas på med at bruge Convert.ToInt32 når det er double
                
            Console.WriteLine("Indtast prisen i kroner per liter:");
                brændstofPris = Convert.ToDouble(Console.ReadLine()); //Pas på med at bruge Convert.ToInt32 når det er double
                
            Console.WriteLine("Indtast hvor mange kilometer bilen har kørt:"); //Min mor skrev . istedet for , 322.000 og det gav error??
        ...
        //Har tilføjet $ så variablerne kan indhentes direkte 
            ...
            Console.WriteLine("Den kører på " + brændstofType + " der koster " + brændstofPris + " DKKR pr liter."); //Hvordan ændre jeg så D for Diesel bliver udskrevet som Diesel istedet for D?????
            Console.WriteLine("Bilen har i alt kørt " + kilometerStand + " KM");
            Console.WriteLine(string.Format("For en tur på {0} KM skal du bruge {1:F2} liter brændstof.", kørtDistance, brændstofbehov)); //hentet fra beregning1 //tilføjet string.Format
            Console.WriteLine(string.Format("Brændstofudgifterne for {0} km er {1:F2} DKK.", kørtDistance, turPris)); //hentet fra beregning2 //tilføjet string.Format
            Console.WriteLine("Bilens kilometerstand efter kørerturen er " + kilometerstandEfter); //Udskriver alle bilens oplysninger og den nye kilometerstand opg2.1.4
        
            Console.WriteLine(); // Tom linje for bedre læsbarhed 
        
        // Tabeloutput (lavet med hjælp) opg.2.2.1
            Console.WriteLine("Bilmærke".PadRight(15) + "| " + "Model".PadRight(15) + "| " + "Kilometertal".PadLeft(14));
            Console.WriteLine(new string('-', 50));
            Console.WriteLine(bilMærke.PadRight(15) + "| " + bilModel.PadRight(15) + "| " + kilometerStand.ToString().PadLeft(5) + " km");
